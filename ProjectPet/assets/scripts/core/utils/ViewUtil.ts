...
export class ViewUtil {
    ...
    public static calculateASpaceToBSpacePos(a: Node, b: Node, aPos: Vec3): Vec3 {
        let world: Vec3 = a.getComponent(UITransform)!.convertToWorldSpaceAR(aPos);
        let space: Vec3 = b.getComponent(UITransform)!.convertToNodeSpaceAR(world);
        return space;
    }
    ...
    public static uniformScale(targetWidth: number, targetHeight: number, defaultWidth: number, defaultHeight: number) {
        ...
        let size = new Size(Math.floor(targetWidth * ratio), Math.floor(targetHeight * ratio));
        return size;
    }

    /** 创建预制节点 */
    public static async createPrefabNode(name: string): Promise<Node> {
        let p: Prefab = await ResLoadManager.Instance.get(name, Prefab);
        if (p) {
            let clone = instantiate(p);
            return clone;
        }
        return null;
    }
    ...
    /** 添加节点动画 */
    public static async addNodeAnimation(name: string, node: Node, onlyOne: boolean = true, isDefaultClip: boolean = false) {
        ...
        let clip = await ResLoadManager.Instance.get(name, AnimationClip) as AnimationClip;
        if (!clip) {
            return;
        }
