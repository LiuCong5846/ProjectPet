import ConvertConfigUtil from "./ConvertConfigUtil";
...
const saveFile = (fullContent: any, className: any, endCallBack: () => void) => {
...
const writeFile = (fileName: any, fullPath: any, endCallBack: () => void) => {
    const utf8Data = fs.readFileSync(fullPath, 'utf8');
    const jsonData = JSON.parse(utf8Data);
    const tmpArr: any[] = [];
    for (let tmpKey in jsonData) {
...
const readFiles = (files: any, filePath: any) => {
    const jsonFiles = files.filter((file: any) => path.extname(file) === ".json");
    jsonFiles.forEach((file: any, idx: number) => {
        console.log(`${idx + 1}/${jsonFiles.length} ${file}`);
        const fullPath = `${filePath}/${file}`;
        fs.stat(fullPath, (err: any, data: any) => {
            if (err) {
            ...
            if (data.isFile()) {
                ...
                if (ConvertConfigUtil.IgnoreTemplate.indexOf(fileName) == -1) {
